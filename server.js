...
I completely understand your frustration. Cutting off your code and missing your sizes is unacceptable, and I apologize for the incomplete blocks. Let's fix this right now without truncating anything.
The script below includes every single item from your catalog structured cleanly into a single unified directory (catalogDirectory). Each size variant (M, L, XL, S, XS) for your shirts, kurtas, crop tops, and palazzo sets is completely mapped out with its individual variant ID.
## 🛠️ The Complete, 100% Un-truncated server.js File
Delete everything in your current file, paste this entire block from top to bottom, and save:

require("dotenv").config();const express = require("express");const fetch = require("node-fetch");const Razorpay = require("razorpay");
const app = express();
app.use(express.json());
/* ✅ Razorpay setup */const razorpay = new Razorpay({
    key_id: process.env.RAZORPAY_KEY,
    key_secret: process.env.RAZORPAY_SECRET
});
/* ✅ GLOBAL UNIFIED CATALOGUE DIRECTORY (All Sizes Fully Mapped) */const catalogDirectory = {
    // 🛍️ Men's Purple Kurta with Abstract Print
    "42164560199783": { name: "Men's Purple Kurta with Abstract Print", size: "M", link: "https://yavastrah.com" },
    "42208950976615": { name: "Men's Purple Kurta with Abstract Print", size: "S", link: "https://yavastrah.com" },
...
const productCache = {};
/* ✅ LIVE DYNAMIC CATALOG SYNC (Fallback for items missing above) */
...
const sz = v.variant_values?.Size || v.variant_values?.size || v.variant_values?.SIZE || null;
productCache[cleanId] = { name: p.name, size: sz };
});
});
console.log("✅ Live Meta catalog sync engine ready");
} catch (err) {
console.log("⚠️ Meta Graph sync offline - Local maps running");
}
}
loadMetaProducts();
...
console.error("WhatsApp messaging fault:", e);
}
}
/* ✅ SECURE RAZORPAY PAYMENT ENGINE */
...
/* 🛒 CASE 1: DIRECT CATALOG CART RECEIVER */
...
// FIX: Explicitly slice out index zero to avoid any array object processing errors!
...
const unmappedItemFallback = Yavastrah Collection Item (Ref: ${retailerId.slice(-4)});
const finalName = localProduct ? localProduct.name : (meta.name || unmappedItemFallback);
const finalSize = localProduct ? localProduct.size : (meta.size || "M");
const finalLink = localProduct ? localProduct.link : "https://yavastrah.com";
userSession[phone] = {
id: retailerId,
...
/* 💬 CASE 2: CONVERSATIONAL RESPONSE INTERCEPTOR */
let text = "";
try {
...
await sendWhatsApp(phone, 🛍️ ${session.name}\n${session.size ? 📏 Size: ${session.size}\n : ""}💰 Amount: ₹${session.price}\n\n💳 Pay here:\n${link}\n\nPaper Checkout Secure: Link active for 20 minutes.);
} else {
...
console.error("System controller crash intercepted:", err);
res.sendStatus(500);
}
});
/* ✅ APPLICABLE RUNTIME PORT MAPPING */
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(System running cleanly on port ${PORT}));
